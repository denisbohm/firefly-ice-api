...
- (void)fireflyIce:(FDFireflyIce *)fireflyIce channel:(id<FDFireflyIceChannel>)channel syncData:(NSData *)data
{
    ...
    if (![@"application/octet-stream" isEqual:[type lowercaseString]]) {
        FDFireflyDeviceLogInfo(@"sync data response: %@", [[NSString alloc] initWithData:responseData encoding:NSUTF8StringEncoding]);
        return;
    }
    FDFireflyDeviceLogInfo(@"sending sync response");
